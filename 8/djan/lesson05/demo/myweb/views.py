...
def stupage(request,pIndex):
	...
	#获取指定pIndex页的数据,意思就是如果pIndex是1,就显示第一页的四条数据，如果是2，就显示第二页的四条数据。
	list2=p.page(pIndex)
	
	#获取页码信息，就是假如整个数据15条，每页4条的话，那么就会有6页，plist就会是123456，这样子的。
	plist=p.page_range
...
def stusearch(request):
	...
	#获取并判断是否添加name条件搜索。注意注意是先写的form表单，这里是获取表单中输入的需要搜的名字
...
def demo1(request):
	...
	#response.set_cookie("username","zhangsan",100)
