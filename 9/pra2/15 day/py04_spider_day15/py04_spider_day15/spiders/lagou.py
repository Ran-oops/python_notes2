...
from py04_spider_day15.items import LagouItem
...
class LagouSpider(CrawlSpider):
    ...
    # 'process_request'是自己定义的，与下面的process_request函数相对应
    ...
    def parse_item(self, response):
        ...
        degree = self.remove_splash(degree)  # remove_splash是下面的函数，用于去掉里面的/
    ...
    def process_date(self, value):
        if '天前' in value:
            # num_pattern是找数字的规则，findall是把所有的数据都找到，search是只找到第一个数据
    ...
    def process_year(self, year):
        if '-' in year:
            ...
            syear = res.group()  # 拿数据的时候用了group
            eyear = res.group()
        else:
            syear = 0
            eyear = 0
        return syear, eyear  # return的是元组，就要用元组接，在61行
