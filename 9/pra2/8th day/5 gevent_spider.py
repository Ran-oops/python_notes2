...
def foo1(task_q):
    while not task_q.empty():
        ...
        for tr in tr_list:
            ...
            data_pub=info[4].text
            sql='insert into tencent_job(job_name,job_number,data_pub) VALUES(%s,%s,%s)'

            #数据库插入
            lock.acquire()
            mydb.execute(sql,(job_name,job_number,data_pub))
            lock.release()

if __name__=='__main__':
    mydb=Mydb('127.0.0.1','root','','temp')
    lock=threading.Lock()
